	...
	public void run() {
		try {
			...
			while ((line = in.readLine()) != null) { // coming from comm in editor
				...
				else if (splitLine[0].equals("SetEndsS")){
					((Segment)(server.getSketch().getShapeMap().get(Integer.parseInt(splitLine[1])))).setEnd(Integer.parseInt(splitLine[4]), Integer.parseInt(splitLine[5]));
					server.broadcast(line);
				}
