	...
	private void handlePress(Point p) {
		...
		if (this.mode == Mode.DRAW) {
			if (this.shapeType.equals("ellipse")) {
				...
				this.curr = new Rectangle(p.x, p.y, this.color);
			}

			else if (this.shapeType.equals("segment")) {
				this.curr = new Segment(p.x, p.y, this.color);
			}

			else if (this.shapeType.equals("polyline")) {
	...
	private void handleDrag(Point p) {
		...
		if (mode == Mode.DRAW) {
			if (shapeType.equals("polyline")) {
				if (movingId != -1){
					comm.send("SetEndsP " + movingId + " " + drawFrom.x + " " + drawFrom.y + " " + p.x + " " + p.y + " " + color);
				}
			}

			else if (shapeType.equals("segment")) {
				//((Segment) curr).setCorners(drawFrom.x, drawFrom.y, p.x, p.y);
				if (movingId != -1){
					comm.send("SetEndsS " + movingId + " " + drawFrom.x + " " + drawFrom.y + " " + p.x + " " + p.y);
				}
			}
