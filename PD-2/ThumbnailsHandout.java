...
public class ThumbnailsHandout extends DrawingGUI {
	...
	private static BufferedImage thumbnailify(BufferedImage image) {
		...
		int scaleX = w/thumbWidth;
		int scaleY = h/thumbHeight;

		for (int c = 0; c < thumbHeight; c++) {
			for (int r = 0; r < thumbWidth; r++) {
				result.setRGB(r, c, image.getRGB(r * scaleX, c * scaleY));
			}
		}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				for (int i=0; i<trows; i++) {
					for (int j=0; j<tcols; j++) {
						images.add(loadImage("pictures/dart/dart"+(i*tcols+j)+".jpg"));
					}
				}
