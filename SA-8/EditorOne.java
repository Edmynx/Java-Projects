...
public class EditorOne extends JFrame {	
	...
	private void handlePress(Point p) {
		...
		if (mode == Mode.DRAW && shapeType.equals("ellipse")) {
			...
			if (mode == Mode.MOVE ) {
				if (shape.contains(p.x, p.y)) moveFrom = p;
			}

			else if (mode == Mode.RECOLOR) {
				if (shape.contains(p.x, p.y)) shape.setColor(color);
			}
	...
	private void handleDrag(Point p) {
		...
		// Be sure to refresh the canvas (repaint) if the appearance has changed
		if (mode == Mode.DRAW) shape.setCorners(drawFrom.x, drawFrom.y, p.x, p.y);
