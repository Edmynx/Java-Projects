	...
	private void handlePress(Point p) {
		...
		if (this.mode == Mode.DRAW) {
			...
			if (this.shapeType.equals("rectangle")) {
				Rectangle temp = new Rectangle(p.x, p.y, p.x, p.y);
		...
		if (this.mode == Mode.MOVE) {
				// request to!
			System.out.println("We just tried to pick a target to move.");
			for (int ID : sketch.getShapeMap().descendingKeySet()) {
				System.out.println("Checking ID " + ID + " to see if we just clicked it.");
				if (sketch.getShapeMap().get(ID).contains(p.x, p.y)) {
					curr = sketch.getShapeMap().get(ID);
					moveFrom = p;
					movingId = ID;
					System.out.println("We just picked a target to move! It's this one: " + movingId);
					break;
				}
			}
...
//			}
		...
		if (this.mode == Mode.RECOLOR) {
			...
			for (int ID : sketch.getShapeMap().descendingKeySet()) {
				...
				System.out.println("Sending through comm a request to Recolor " + movingId + " to " + color.getRGB());
				comm.send("Recolor " + movingId + " " + color.getRGB()); // make sure this has appropriate spaces
				repaint();
			}
		}
	...
	private void handleDrag(Point p) {
		...
		if (mode == Mode.DRAW) {
			...
			if ((shapeType.equals("polyline"))) {
				if (movingId != -1){
					comm.send("SetCornersP " + movingId + " " + drawFrom.x + " " + drawFrom.y + " " + p.x + " " + p.y + " " + color);
				}
			}
			...
			//curr.moveBy(moveX, moveY);
			moveFrom.setLocation(p.getX(), p.getY());

			System.out.println("We just communicated to the SSC to move.");
