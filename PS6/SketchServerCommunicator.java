 ...
 * Modified by Edmund Aduse Poku (worked with Arjun)
 */
	...
	public void run() {
		try {
			...
			for (int id: server.getSketch().getShapeMap().navigableKeySet()){
				...
				System.out.println("These are the things already in our Map!\n" + shapestring + " which corresponds to ID " + id);
				send(shapestring + " " + id);
			}
			...
			while ((line = in.readLine()) != null) { // coming from comm in editor
				...
				else if (splitLine[0].equals("SetCornersP")){
					((Polyline)(server.getSketch().getShapeMap().get(Integer.parseInt(splitLine[1])))).addSegment(new Segment(Integer.parseInt(splitLine[2]), Integer.parseInt(splitLine[3]), Integer.parseInt(splitLine[4]), Integer.parseInt(splitLine[5]), new Color(Integer.parseInt(splitLine[6]))));
					server.broadcast(line);
				}

				else if (splitLine[0].equals("SetCornersE")){
					System.out.println(server.getSketch().getShapeMap().keySet());
					if (server.getSketch().getShapeMap().containsKey(Integer.parseInt(splitLine[1]))) {
