 ...
 * Modified by Edmund (worked with Arjun)
 */
	...
	public void run() {
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (lineSplit[0].equals("Move")){ // Move MovingID x y
				...
				else if (lineSplit[0].equals("SetCornersP")){
					((Polyline)(editor.getSketch().getShapeMap().get(Integer.parseInt(lineSplit[1])))).addSegment(new Segment(Integer.parseInt(lineSplit[2]), Integer.parseInt(lineSplit[3]), Integer.parseInt(lineSplit[4]), Integer.parseInt(lineSplit[5]), new Color(Integer.parseInt(lineSplit[6]))));
				}
